...
export default function BillingPreviousPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = await apiClient<any>("/api/billing");
        
        if (!canceled) { 
          const billArray = Array.isArray(data) ? data : (data.items || []);
          setBills(billArray); 
          setLoading(false); 
          return; 
        }
      } catch (e) {
        console.error("Backend fetch failed, falling back to local storage", e);
      }

      // fallback -> localStorage if API fails or errors out
      try {
        ...
        if (!canceled) setError("Failed to load local bills");
        setBills([]);
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    const result = bills.filter(b => {
      ...
      if (!t) return true;
      return (
        (b.clientName || "").toLowerCase().includes(t) || 
        String(b.billNumber || "").toLowerCase().includes(t) || 
        String(b.id || "").toLowerCase().includes(t)
      );
    });
        ...
        <div style={{ background: "#fff", borderRadius: 10, padding: 20 }}>
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
              <button 
                onClick={goCreateNew} 
                style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #0b7a53", background: "#0b7a53", color: "#fff", cursor: "pointer" }}
              >
                CREATE NEW
              </button>
            </div>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 12, borderBottom: "1px solid #eee", paddingBottom: 8 }}>
            {["Billed", "Credited", "Draft", "Cancelled", "Proforma Draft", "All"].map(s => (
              <div 
                ...
                style={{ 
                  cursor: "pointer", 
                  paddingBottom: statusFilter === s ? 6 : 8, 
                  borderBottom: statusFilter === s ? "2px solid #0b7a53" : "2px solid transparent", 
                  color: statusFilter === s ? "#0b7a53" : "#6b7280", 
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr 1fr 1fr", gap: 12, alignItems: "center", fontWeight: 700, padding: "8px 6px", borderBottom: "1px solid #eee", fontSize: 13 }}>
            <div>BILL NUMBER</div>
            <div>CLIENT</div>
            <div>DATE</div>
          ...
          {loading ? (
            ...
            <div style={{ padding: 28, textAlign: "center", color: "red" }}>{error}</div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: 28, textAlign: "center", color: "#777" }}>There are no items to display...</div>
          ) : (
            filtered.map(b => (
              <div key={String(b.id)} style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr 1fr 1fr", padding: "10px 6px", borderBottom: "1px solid #f3f4f6", gap: 12, alignItems: "center", fontSize: 14 }}>
                <div 
                  style={{ color: "#0b7a53", cursor: "pointer", fontWeight: 600 }} 
                  onClick={() => router.push(`/billing/bills/${b.id}`)}
                >
                  {b.billNumber || b.id}
                </div>
                ...
                <div style={{ textAlign: "center" }}>
                   <span style={{ 
                     ...
                     borderRadius: 12, 
                     fontSize: 11, 
                     background: (b.status||"").toLowerCase().includes("paid") ? "#e1f5ec" : "#f3f4f6",
                     color: (b.status||"").toLowerCase().includes("paid") ? "#0b7a53" : "#666"
                   }}>
                     {b.status}
                   </span>
                </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 12, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button 
                disabled={page * pageSize >= bills.length} 
                onClick={() => setPage(p => p + 1)} 
                style={{ padding: "6px 12px", cursor: page * pageSize >= bills.length ? "not-allowed" : "pointer" }}
              >
                Next
              </button>
            </div>
