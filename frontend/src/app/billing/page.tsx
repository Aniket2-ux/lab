  ...
  export default function BillingPage() {
    ...
    const [referrerModalOpen, setReferrerModalOpen] = useState(true);
    useEffect(() => {
  console.log("referrerModalOpen =", referrerModalOpen);
}, [referrerModalOpen]);
    ...
    // load referrers (API first, localStorage fallback)
useEffect(() => {
  const loadReferrers = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/referrers`);
          ...
          <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", padding: 20 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
              ...
              <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                <button onClick={() => { setActiveTopTab("New Bill"); window.scrollTo(0, 0); router.push("/billing"); }} style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #0b7a53", background: "#0b7a53", color: "#fff" }}>CREATE NEW</button>
          ...
          <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 4px 12px rgba(0,0,0,0.05)", padding: 20, marginBottom: 16 }}>
              ...
              <div style={{ flex: 1.2 }}>
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button
                      type="button"
                      onClick={() => { alert("ADD CLICKED"); setReferrerModalOpen(true);}}
                      style={{ padding: "6px 12px", borderRadius: 4, border: "1px solid #0b7a53", background: "#e6f4ef", color: "#0b7a53", fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                    >
                      ADD
                    </button>
                  </div>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={item.id} style={{ display: "grid", gridTemplateColumns: "5fr 1.3fr 0.8fr 1fr 1.3fr 1.3fr 40px", columnGap: 6, marginBottom: 6 }}>
                      ...
                      <button type="button" onClick={() => removeItemRow(item.id)} style={{ border: "none", background: "transparent", cursor: "pointer", color: "#dc2626", fontSize: 18 }}>×</button>
                    </div>
                  );
                })}
                <button type="button" onClick={addItemRow} style={{ marginTop: 8, padding: "8px 14px", borderRadius: 4, border: "1px solid #0b7a53", background: "#e6f4ef", color: "#0b7a53", fontSize: 13, cursor: "pointer" }}>ADD BILL ITEM</button>
              </div>
              ...
              <div style={{ flex: 0.9, fontSize: 13, display: "grid", rowGap: 8 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.4fr 3fr", alignItems: "center", columnGap: 6 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <input style={{ ...inputStyle, flex: 1 }} type="number" value={discountValue} onChange={(e) => setDiscountValue(Number(e.target.value) || 0)} />
                  </div>
                </div>
                <RowLabelValue label="Referral Commission" value={referralCommission} />
              ...
              <div style={{ flex: 1.4, fontSize: 13 }}>
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <input style={{ ...inputStyle, width: 60, textAlign: "center" }} type="number" value={remindValue} onChange={(e) => setRemindValue(Number(e.target.value) || 0)} />
                    <button type="button" onClick={() => setRemindValue((v) => v + 1)} style={circleButton}>+</button>
                    {(["DAYS", "WEEK", "MONTH", "YEARS"] as const).map((u) => (
                      <button key={u} type="button" onClick={() => setRemindUnit(u)} style={{ padding: "5px 10px", borderRadius: 4, border: remindUnit === u ? "1px solid #0b7a53" : "1px solid #e5e7eb", background: remindUnit === u ? "#e6f4ef" : "#ffffff", fontSize: 11, cursor: "pointer" }}>{u}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <div style={{ marginBottom: 4, fontWeight: 600 }}>Remarks</div>
                  <textarea style={{ ...inputStyle, minHeight: 80 }} value={remarks} onChange={(e) => setRemarks(e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div style={{ position: "sticky", bottom: 0, left: 0, right: 0, marginLeft: -24, marginRight: -24, padding: "10px 24px", background: "#ffffff", borderTop: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button type="button" style={bottomButtonGreen} onClick={handleFinalise} disabled={loadingAction}>{loadingAction ? "PROCESSING..." : "FINALISE"}</button>
              </div>
            </div>
